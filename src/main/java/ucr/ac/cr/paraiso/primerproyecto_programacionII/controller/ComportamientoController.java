...
public class ComportamientoController {
    ...
    private void llenarComboBox() {
        ...
        if (patronXMLData != null && clasificacionXMLData != null) {
            try {
                String idClasificacionComportamiento = obtenerIdClasificacionComportamiento();
                if (idClasificacionComportamiento != null) {
                    for (Patron patron : patronXMLData.obtenerPatrones()) {
                        if (patron.getIdClasificacion().equals(idClasificacionComportamiento)) {
                            nombresPatrones.add(patron.getIdPatron() + " - " + patron.getName());
                        }
                    }
                    cBoxID.setItems(nombresPatrones);
                } else {
                    mostrarMensajeError("Clasificación 'Comportamiento' no encontrada.");
                }
            } catch (Exception e) {
    ...
    private String obtenerIdClasificacionComportamiento() {
        try {
            for (Clasificacion clasificacion : clasificacionXMLData.obtenerClasificaciones()) {
                if ("Comportamiento".equals(clasificacion.getNameClasificacion())) {
                    return clasificacion.getIdClasificacion();
                }
            }
        } catch (Exception e) {
            mostrarMensajeError("Error al obtener la clasificación 'Comportamiento'.");
            e.printStackTrace();
        }
        return null;
    }
